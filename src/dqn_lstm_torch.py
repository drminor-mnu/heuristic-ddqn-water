...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 00:47:22 2024
...
hidden_dim = 64 # ther number of nodes for a layer
...
def train(train_inps, valid_inps, epochs:int=1, minutes:int=1):
    ...
    policy_model = DqnLSTM(input_dim=5, hidden_dim=hidden_dim, 
    ...
    torch.save(policy_model.state_dict(), '../trained_models/water_dqn_lstm.pkl')
...
def test_model(model, test_inp, minutes:int=1):
    ...
    if model == None:
        ...
        model = DqnLSTM(input_dim=5, hidden_dim=hidden_dim, 
                        output_dim=len(actions), 
                        num_layers=num_layers).to(device)
        model.load_state_dict(torch.load('../trained_models/water_dqn_lstm.pkl'))
        model.to(device)
        model.eval()
...
if __name__ == '__main__':
    ...
    model, losses = train(train_list, valid_list, minutes=2)
    plot_losses(losses)
    test_list = ['data/gasan/30year/30yr_0060m_h160.inp']
