...
def data_split(trate:float=0.7, vrate:float=0.1):
    ...
    model_path = f"../trained_models/dqn_gru_regular_\
    w{int(params['weights'][0])}_\
    w{int(params['weights'][1])}_\
    w{int(params['weights'][2])}_\
    w{int(params['weights'][3])}_min2_{trate}.pkl"
    Parameters
    ----------
    trate : TYPE, optional
...
def seed_with_demon(replay:deque, train_inps:list, n_episodes:int=20, **params)->list:

    minutes = params.get('minutes', 2)

    # (state_prev, action, reward, state_post, done)
    for inp in train_inps[:n_episodes]:
...
def train(train_inps, valid_inps=None, model_path:str=None, **params):
    """
    ...
    for epoch in range(epochs):
        ...
        random.shuffle(train_inps)
        losses = []
        train_episode_returns = []   # <- train total reward save for each episode
        
        #=============================================================#
        # Fill replay buffer with demonstraion from Genetic Algorithm #
        #=============================================================#
        seed_with_demon(replay, train_inps, n_episodes=20)
        print("Demonstration seeding completed")
        train_inps = train_inps[n_episodes:]
        num_samples = len(train_inps)
    ...
    if model_path is None:
        model_path = '../trained_models/dqn_gru_demon_default.pkl'
...
if __name__ == '__main__':
    ...
    model_path = '../trained_models/dqn_gru_demon_w1_w1_w1_min2_epo20.pkl'
